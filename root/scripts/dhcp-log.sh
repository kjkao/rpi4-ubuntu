...
#grep -h -a -e ': DHCPOFFER' /var/log/syslog.1 /var/log/syslog | while read L ; do
#grep -a -e ': DHCPACK' /var/log/syslog.1 | cut -d ' ' -f 6,8,10,1-3 | while read L ; do
#  IP=`echo $L | cut -d ' ' -f 5`
#  MC=`echo $L | cut -d ' ' -f 6`
#  TS=`echo $L | cut -d ' ' -f 1-3`
grep -h -a -e ": $TKN" /var/log/syslog.1 /var/log/syslog | while read L ; do
  LA=($L)
  IP=${LA[7]}
  MC=${LA[9]}
  TS="${LA[0]} ${LA[1]} ${LA[2]}"
